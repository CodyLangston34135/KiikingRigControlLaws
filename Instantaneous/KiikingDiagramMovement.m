...
for i = 1:nStep
    ...
    if thetaDotStep(i,1)==0 || thetaDotStep(i,1)*thetaStep(i,1)<0
        ...
        endCond = [0 3];
        [thetaDamp,thetaDotDamp,time,overflow] = runODE(thetaStep(i,1),thetaDotStep(i,1),endCond,numCycle,mu,g,Swing,[Squat; Stand],moveTime,odeTime,options);   % Damped Ideal

        % Phase Diagram
        figure(1)
        plot(thetaDamp,thetaDotDamp,'m');
